...
class PlanItineraryLeg {
  PlanItineraryLeg({
    ...
    transportMode = getTransportMode(mode, optional: routeLongName);
  }
...
TransportMode getTransportMode(String enumText, {String optional}) =>
    TransportModeExtension.names.keys.firstWhere(
      (key) => key.name == enumText,
      orElse: () => optional != null
          ? _getSpecificTransport(
              optional,
            )
          : TransportMode.walk,
    );

TransportMode _getSpecificTransport(String enumText) {
  final String enumType = enumText?.toUpperCase() ?? "";
  return enumType.contains(TransportMode.trufi.name)
      ? TransportMode.trufi
