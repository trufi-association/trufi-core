...
/// Unit tests for OTP 2.4 response parser.
void main() {
  group('Otp24ResponseParser', () {
    late Map<String, dynamic> fixtureJson;

    setUpAll(() {
      final file = File('test/fixtures/otp_2_4_response.json');
      fixtureJson = jsonDecode(file.readAsStringSync()) as Map<String, dynamic>;
    });

    test('parsePlan returns valid Plan from fixture', () {
      final plan = Otp24ResponseParser.parsePlan(fixtureJson);
    ...
    test('parsePlan parses nested route info from GraphQL response', () {
      final plan = Otp24ResponseParser.parsePlan(fixtureJson);
    ...
    test('parsePlan parses nested agency info from route', () {
      final plan = Otp24ResponseParser.parsePlan(fixtureJson);
      final busLeg = plan.itineraries!.first.legs[1];
    ...
    test('parsePlan parses places with stop info', () {
      final plan = Otp24ResponseParser.parsePlan(fixtureJson);
      final busLeg = plan.itineraries!.first.legs[1];
    ...
    test('parsePlan parses intermediate places with stop info', () {
      final plan = Otp24ResponseParser.parsePlan(fixtureJson);
      final busLeg = plan.itineraries!.first.legs[1];
      ...
      expect(intermediatePlace.lat, equals(-17.40));
      expect(intermediatePlace.lon, equals(-66.18));
    });
    ...
    test('parsePlan returns empty Plan when plan data is null', () {
      ...
      final plan = Otp24ResponseParser.parsePlan(emptyJson);
    ...
    test('parsePlan handles missing optional fields gracefully', () {
      ...
      final plan = Otp24ResponseParser.parsePlan(minimalJson);
    ...
    test('parsePlan handles null route in leg', () {
      final jsonWithoutRoute = {
        'plan': {
          'from': {'name': 'A', 'lat': -17.0, 'lon': -66.0},
          'to': {'name': 'B', 'lat': -17.1, 'lon': -66.1},
          'itineraries': [
